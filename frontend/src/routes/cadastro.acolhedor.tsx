...
function Page() {
  ...
  const onSubmit = (d: CadastroAcolhedorInput) => {
    try { cadastrarAcolhedor(d); toast.success("Acolhedor cadastrado!"); navigate({ to: "/dashboard" }); }
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-10">
      ...
      <form onSubmit={f.handleSubmit(onSubmit)} className="mt-6 space-y-4">
        <div><Label>Nome completo</Label><Input {...f.register("nomeCompleto")} /></div>
        <div className="grid gap-4 sm:grid-cols-2">
          <div><Label>E-mail</Label><Input type="email" {...f.register("email")} /></div>
          <div><Label>Senha</Label><Input type="password" {...f.register("senha")} /></div>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <div><Label>CPF (11 dígitos)</Label><Input {...f.register("cpf")} /></div>
          <div><Label>Telefone</Label><Input {...f.register("telefone")} /></div>
        </div>
        <div><Label>Endereço</Label><Input {...f.register("endereco")} /></div>
        <div className="grid gap-4 sm:grid-cols-2">
