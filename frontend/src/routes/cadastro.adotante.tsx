...
export const Route = createFileRoute("/cadastro/adotante")({
  head: () => ({
    meta: [
      { title: "Cadastro de adotante — AdoptPlace" },
      { name: "description", content: "Crie sua conta de adotante." },
    ],
  }),
  component: Page,
});
...
function Page() {
  ...
  const onSubmit = async (d: CadastroAdotanteInput) => {
    try {
      await cadastrarAdotante(d);
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Após o cadastro você será direcionado para a triagem obrigatória.
      </p>
      <form onSubmit={f.handleSubmit(onSubmit)} className="mt-6 space-y-4">
        <Field label="Nome completo" error={f.formState.errors.nomeCompleto?.message}>
          <Input {...f.register("nomeCompleto")} />
        </Field>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="E-mail" error={f.formState.errors.email?.message}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="CPF (11 dígitos)" error={f.formState.errors.cpf?.message}>
            <Input {...f.register("cpf")} />
          </Field>
          <Field label="Telefone" error={f.formState.errors.telefone?.message}>
