...
export const cpfSchema = z.string().trim()
  .refine(validCpf, "Informe um CPF válido.")
  .transform(digits);

export const cnpjSchema = z.string().trim()
...
export const personNameSchema = z.string().trim()
  ...
  .refine(
    (value) => /[A-Za-zÀ-ÖØ-öø-ÿ]{2}/u.test(value) && !/\d/u.test(value),
    "Informe um nome válido, sem números.",
  );

export const addressSchema = z.string().trim()
