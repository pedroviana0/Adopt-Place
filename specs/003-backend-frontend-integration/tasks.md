...
- [X] T071 [US3] Integrate real owner animal management in `frontend/src/lib/data/animais.ts`, `frontend/src/routes/_authenticated.dashboard.animais.*.tsx`, `frontend/src/components/app/AnimalForm.tsx`, and `frontend/src/components/app/RelatedAnimalsPanel.tsx` (Owner: Arthur; depends on T070) — done: real CRUD, upload/photo management, relationships and search were homologated against the isolated backend
- [X] T072 [US3] Remove owner animal management mock/localStorage dependency from completed functions in `frontend/src/lib/data/animais.ts` (Owner: Arthur; depends on T071) — done: the final mock helpers and storage modules were removed after SC-006 certification
...
- [X] T080 [US3] Integrate real health and owner request flows in `frontend/src/lib/data/saude.ts`, `frontend/src/lib/data/solicitacoes.ts`, `frontend/src/routes/_authenticated.dashboard.solicitacoes.*.tsx`, `frontend/src/routes/_authenticated.dashboard.animais.$animalId.tsx`, `frontend/src/components/app/HealthPanel.tsx`, and `frontend/src/components/app/TriagemReadOnly.tsx` (Owner: Arthur; depends on T079) — done: Issue #45/#46, PR #88 (merged)
- [X] T081 [US3] Remove completed health/request mock/localStorage dependency from `frontend/src/lib/data/saude.ts` and relevant functions in `frontend/src/lib/data/solicitacoes.ts` (Owner: Arthur; depends on T080) — done: Issue #45/#46, PR #88 (merged); out-of-scope mock helpers intentionally kept
- [X] T082 [US3] Update matrix health/request rows to `flow complete` with validation evidence (Owner: Pedro; depends on T081) — done: SC-006 exercised owner decision/privacy and basic health create/read/delete against isolated PostgreSQL through the frontend proxy
...
- [X] T083 [US4] Audit feature 002 backend behavior against `specs/002-health-dashboard-chat/spec.md`, `lib/actions/cuidados-planejados.ts`, `lib/actions/documentos-saude.ts`, `lib/actions/mensagens.ts`, `lib/queries/health-dashboard.ts`, `lib/queries/operational-dashboard.ts`, and `lib/queries/mensagens.ts`; record pass/partial/blocked status in `specs/003-backend-frontend-integration/integration-matrix.md` (Owner: Pedro) — done (Issue #48): "Feature 002 Audit" section; rules pass, HTTP contracts partial/absent
- [X] T084 [P] [US4] Add failing backend tests for CONSULTA never becoming clinical history and planned-care idempotency in `__tests__/actions/cuidados-planejados.test.ts` and `__tests__/queries/health-dashboard.test.ts` (Owner: Pedro; depends on T083) — done (Issue #49): rules already covered by `__tests__/actions/cuidados-planejados.test.ts`; added HTTP-level coverage in `__tests__/api/health-center.test.ts`
- [X] T085 [P] [US4] Add failing backend tests for document privacy and dashboard ownership isolation in `__tests__/actions/documentos-saude.test.ts` and `__tests__/queries/operational-dashboard.test.ts` (Owner: Pedro; depends on T083) - done (Issues #50/#51): existing action/query coverage retained; HTTP boundary and current-account revalidation added in `__tests__/api/operational-dashboard.test.ts`, `__tests__/api/health-documents.test.ts`, and document upload/action tests
- [X] T086 [P] [US4] Add failing backend tests for chat only after approval, participant authorization, and archived read-only behavior in `__tests__/actions/mensagens.test.ts` and `__tests__/queries/mensagens.test.ts` (Owner: Pedro; depends on T083) - done (Issue #52): existing lifecycle/action/query coverage retained; HTTP authorization, archived send and safe polling added in `__tests__/api/chat.test.ts` and `__tests__/queries/mensagens-polling.test.ts`

### Implementation for User Story 4

- [X] T087 [US4] Define only the audited feature 002 health-center HTTP contract in `specs/003-backend-frontend-integration/contracts/http-contract-inventory.md`; mark its matrix row `contract defined` only with evidence and without presuming a missing endpoint or enum value (Owner: Pedro; depends on T083) — done (Issue #49): `HEALTH-CENTER-01` section added
- [X] T088 [US4] Define only the audited feature 002 operational-dashboard HTTP contract in `specs/003-backend-frontend-integration/contracts/http-contract-inventory.md`; mark its matrix row `contract defined` only with evidence and without presuming a missing endpoint or enum value (Owner: Pedro; depends on T087) - done (Issue #50): `OPERATIONAL-DASHBOARD-01`
- [X] T089 [US4] Define only the audited feature 002 health-document HTTP contract in `specs/003-backend-frontend-integration/contracts/http-contract-inventory.md`; mark its matrix row `contract defined` only with evidence and without presuming a missing endpoint or enum value (Owner: Pedro; depends on T088) - done (Issue #51): `HEALTH-DOCUMENTS-01`
- [X] T090 [US4] Define only the audited feature 002 chat HTTP contract in `specs/003-backend-frontend-integration/contracts/http-contract-inventory.md`; mark its matrix row `contract defined` only with evidence and without presuming a missing endpoint or enum value (Owner: Pedro; depends on T089) - done (Issue #52): `ADOPTION-CHAT-01`
- [X] T091 [US4] Implement only the audited health-center contract from T087 in existing `lib/actions/cuidados-planejados.ts` and `lib/queries/health-dashboard.ts` or route handlers documented by T087, preserving the rule that CONSULTA never becomes clinical history (Owner: Pedro; depends on T084, T087 and requires the health-center matrix row at `contract defined`) — done (Issue #49): route handlers under `app/api/saude/visao-geral`, `app/api/saude/agenda`, `app/api/saude/cuidados/**` wrapping the existing action/query; CONSULTA-not-history and idempotency preserved. Matrix row `backend ready`.
- [X] T092 [US4] Implement only the audited operational-dashboard contract from T088 in existing `lib/queries/operational-dashboard.ts` or a route handler documented by T088, proving ownership isolation (Owner: Pedro; depends on T085, T088 and requires the dashboard matrix row at `contract defined`) - done (Issue #50): protected `GET /api/dashboard/operacional`, current owner context and aggregate isolation tests
- [X] T093 [US4] Implement only the audited health-document contract from T089 in existing `lib/actions/documentos-saude.ts`, related queries/schemas, or a route handler documented by T089, proving document privacy (Owner: Pedro; depends on T085, T089 and requires the health-document matrix row at `contract defined`) - done (Issue #51): owner-scoped list/detail/delete, revalidated UploadThing authorization and unique upload IDs
- [X] T094 [US4] Implement only the audited chat contract from T090 in existing `lib/actions/mensagens.ts`, `lib/queries/mensagens.ts`, and `app/api/mensagens/[id]/route.ts` or a route handler documented by T090, preserving access only after approval and read-only state after adoption conclusion (Owner: Pedro; depends on T086, T090 and requires the chat matrix row at `contract defined`) - done (Issue #52): participant list/detail/send/read endpoints, archived 409 and safe polling DTO
- [X] T095 [US4] Update feature 002 matrix rows to `backend ready` individually only where T091-T094 tests and acceptance evidence pass (Owner: Pedro; depends on T091-T094) - done (Issue #53): four backend slices certified individually; 208 tests, typecheck, lint, Prisma validation and backend/frontend production builds passed; frontend completion remains pending
- [X] T096 [US4] Audit missing frontend routes/components for health center, dashboard, documents, and chat in `frontend/src/routes/` and `frontend/src/components/app/`; record required Arthur Issue pairs and exact affected areas in `specs/003-backend-frontend-integration/integration-matrix.md` without creating Issues (Owner: Arthur; depends on T095) — done (Issue #54): "Feature 002 Frontend Audit" section
- [X] T097 [US4] Align health, dashboard, document, and chat types/enums in `frontend/src/lib/domain/` and affected `frontend/src/lib/data/` type boundaries to the contracts audited by T083 and defined by T087-T090; do not duplicate divergent types or presume enum values (Owner: Arthur; depends on T083, T087-T090, T096) — done (Issue #54): enums.ts aligned to Prisma (5 TipoRegistroSaude + TipoCuidadoPlanejado/StatusCuidadoPlanejado/TipoDocumentoSaude/StatusConversaAdocao)
- [X] T098 [US4] Integrate only the real health-center flow in the exact `frontend/src/routes/`, `frontend/src/components/app/`, and data files identified by T096, keeping CONSULTA out of clinical history (Owner: Arthur; depends on T095-T097) — done (Issue #55): `frontend/src/lib/data/cuidados.ts` + `_authenticated.dashboard.saude.{tsx,index.tsx}` + nav; consumes HEALTH-CENTER-01; CONSULTA completed with no health record
- [X] T099 [US4] Integrate only the real operational-dashboard flow in the exact `frontend/src/routes/`, `frontend/src/components/app/`, and data files identified by T096 (Owner: Arthur; depends on T095-T097) — done (Issue #56): `frontend/src/lib/data/dashboard.ts` + reworked `_authenticated.dashboard.index.tsx` consuming `GET /api/dashboard/operacional`
- [X] T100 [US4] Integrate only the real health-document flow in the exact `frontend/src/routes/`, `frontend/src/components/app/`, and data files identified by T096 (Owner: Arthur; depends on T095-T097) — done (Issue #57): `frontend/src/lib/data/documentos.ts` + `_authenticated.dashboard.documentos.{tsx,index.tsx}` + nav; list/filter/open/delete over `/api/saude/documentos` and upload via the `healthDocument` uploadthing route
- [X] T101 [US4] Integrate only the real chat flow in the exact `frontend/src/routes/`, `frontend/src/components/app/`, and data files identified by T096, showing chat only after approval and disabling sends after adoption conclusion (Owner: Arthur; depends on T095-T097) — done (Issue #58): `frontend/src/lib/data/mensagens.ts` + `_authenticated.dashboard.mensagens.{tsx,index.tsx,$conversaId.tsx}` + nav unread badge; polling, sends disabled when archived (`canSend`). Adopter-side entry (FR-071) recorded as follow-up (dashboard is responsible-only).
- [X] T102 [US4] Remove only completed health-center mock/localStorage dependencies from the exact `frontend/src/lib/data/` modules identified by T096 after T098 works (Owner: Arthur; depends on T098) — n/a (Issue #55): the health center is a new surface with no prior mock; nothing to remove. (`saude.ts` mocks belong to the dashboard summary / orphan card, tracked under #56/#63.)
- [X] T103 [US4] Remove only completed dashboard mock/localStorage dependencies from the exact `frontend/src/lib/data/` modules identified by T096 after T099 works (Owner: Arthur; depends on T099) — done (Issue #56): the dashboard home no longer imports `listAnimais`/`listSolicitacoesPorResponsavel`/`alertasProximos`; those mock helpers now have no consumer in the dashboard flow (final `db.ts`/`seed.ts` removal remains #63)
- [X] T104 [US4] Remove only completed health-document mock/localStorage dependencies from the exact `frontend/src/lib/data/` modules identified by T096 after T100 works (Owner: Arthur; depends on T100) — n/a (Issue #57): health documents are a new surface with no prior mock; nothing to remove
- [X] T105 [US4] Remove only completed chat mock/localStorage dependencies from the exact `frontend/src/lib/data/` modules identified by T096 after T101 works (Owner: Arthur; depends on T101) — n/a (Issue #58): chat is a new surface with no prior mock; nothing to remove
- [X] T106 [US4] Update feature 002 matrix rows to `flow complete` individually with evidence for CONSULTA rule, dashboard ownership, document privacy, chat release, archived read-only state, frontend type alignment, and per-flow mock removal (Owner: Pedro; depends on T102-T105) — done: SC-006 certified all four flows against isolated PostgreSQL, including real UploadThing upload/list/open/delete and active-to-archived chat
- [X] T124 [US4] Add adopter chat list/detail routes outside the responsible-only dashboard, link approved/completed requests to their conversation, and show the adopter unread indicator using the existing participant-scoped HTTP contracts (Owner: Arthur; depends on T101) — done (Issue #92): shared conversation list/detail components serve both audiences; `/mensagens` is adopter-only; approved/completed requests link by `requestId`; frontend typecheck, focused lint, and production build pass
...
- [X] T111 [US3] Integrate real admin users in `frontend/src/lib/data/usuarios.ts` and `frontend/src/routes/_authenticated.dashboard.admin.usuarios.tsx` (Owner: Arthur; depends on T110) — done (Issue #61): `fetchAdminUsuarios`/`setUsuarioAtivo` over ADMIN-01; admin route uses React Query
- [X] T112 [US3] Remove completed admin mock/localStorage dependency from admin functions in `frontend/src/lib/data/usuarios.ts` (Owner: Arthur; depends on T111) — done (Issue #61): removed mock `listUsuarios` and `setAtivo`
- [X] T113 [US3] Update matrix admin rows to `flow complete` with validation evidence (Owner: Pedro; depends on T112) — done (Issue #62): ADMIN-01 uses the real protected backend, frontend React Query consumption has no admin mock/localStorage path, focused admin tests pass (4/4), combined admin/chat validation passes (10/10), frontend typecheck/focused lint/build pass, and DTO review confirms no password hash/private profile exposure
...
- [X] T114 [US5] Audit remaining runtime dependencies on `frontend/src/lib/data/db.ts` and `frontend/src/lib/data/seed.ts`; list each dependent flow in `specs/003-backend-frontend-integration/integration-matrix.md` (Owner: Arthur; depends on T043, T052, T060, T073, T082, T106, T113) — done: the dependency graph was reviewed after SC-006 and only dead/orphan mock consumers remained
- [X] T115 [US5] Remove or isolate `frontend/src/lib/data/db.ts` and `frontend/src/lib/data/seed.ts` as dev-only fixtures only after all dependent matrix rows are `flow complete` (Owner: Arthur; depends on T114) — done: both runtime storage files, orphan helpers/components and the uncontracted historical adopters route were removed without inventing an endpoint
- [X] T116 [US5] Verify `frontend/` has no direct Prisma/PostgreSQL imports or DB credentials by scanning `frontend/src/` and record result in `specs/003-backend-frontend-integration/integration-matrix.md` (Owner: Pedro; depends on T115) — done: final scan found no Prisma/PostgreSQL import, connection string or DB credential
- [X] T117 [US5] Confirm no files under `legacy/frontend-antigo/` received feature 003 functionality and record result in `specs/003-backend-frontend-integration/integration-matrix.md` (Owner: Pedro; depends on T115) — done: final diff remains empty under `legacy/frontend-antigo/`
...
- [X] T118 Create the homologation checklist in `specs/003-backend-frontend-integration/quickstart.md` with representative adopter, adopter without screening, organization, foster and admin accounts; define SC-006 timing steps for login/session reload/logout, public showcase filter-to-detail, profile edit, screening-to-request, responsible request decision, animal create-or-edit, health record operation, dashboard review, health-document access, approved/read-only chat, and admin activation/deactivation, recording role, timestamps, elapsed time, environment, result, and evidence reference (Owner: Pedro) — done (Issue #65): quickstart records isolated-environment rules, six account aliases, evidence fields, eleven timed flows, under-three-minute rule, storage/persistence check, and explicit no-secret/no-original-database constraints
- [X] T119 Run backend validation commands `npm test`, `npm run typecheck`, `npm run lint`, `npm run prisma:validate`, and `npm run build`; record output summary in `specs/003-backend-frontend-integration/integration-matrix.md` (Owner: Pedro; depends on T118) — done (Issue #66): 44 test files/212 tests pass; typecheck, isolated lint rerun, Prisma validation, and production build pass
- [X] T120 Run frontend validation commands `npm --prefix frontend run lint` and `npm --prefix frontend run build`; record output summary in `specs/003-backend-frontend-integration/integration-matrix.md` (Owner: Arthur; depends on T118) — done (Issue #67): production build and explicit `tsc --noEmit` pass; full lint reproduces the preexisting repository-wide CRLF/Prettier debt, while focused semantic lint of every file changed by Issues #62/#92 has 0 errors (one preexisting Fast Refresh warning in `AnimalFilters.tsx`)
- [X] T121 Validate no seed, reset, or migration was run against the original database during feature 003 and record confirmation in `specs/003-backend-frontend-integration/integration-matrix.md` (Owner: Pedro) — done (Issue #66): command audit confirms this execution ran no seed, reset, migration, Prisma write, or database data command; only `prisma validate` was used
- [X] T122 Execute the manual SC-006 checklist in homologation and record evidence that every selected small flow completed in under 3 minutes, without requiring an automated duration test (Owner: Pedro; depends on T043, T052, T060, T073, T081, T105, T113, T118, T124; T082 and T106 are promoted only from the evidence produced here) — done: all selected flows passed through the frontend same-origin proxy against disposable PostgreSQL 16; each completed in under 12 seconds
- [X] T123 Perform final matrix review and mark only completed feature 003 rows as `flow complete` in `specs/003-backend-frontend-integration/integration-matrix.md`; do not alter feature 001 T104 in `specs/001-animal-adoption-management/tasks.md` (Owner: Pedro; depends on T119-T122) — done: matrix reviewed from recorded tests and SC-006 evidence; feature 001 T104 remains untouched and pending
