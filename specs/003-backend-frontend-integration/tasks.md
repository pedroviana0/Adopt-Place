...
- [~] T082 [US3] Update matrix health/request rows to `flow complete` with validation evidence (Owner: Pedro; depends on T081) — partial (Issue #47): REQUEST-OWNER-01 and HEALTH-BASIC-01 advanced to `frontend integrated` with real evidence (PR #88; tsc/eslint/build). Promotion to `flow complete` awaits the manual homologation round-trip (SC-006/T122), not runnable without the homologation database.
...
- [~] T106 [US4] Update feature 002 matrix rows to `flow complete` individually with evidence for CONSULTA rule, dashboard ownership, document privacy, chat release, archived read-only state, frontend type alignment, and per-flow mock removal (Owner: Pedro; depends on T102-T105) — partial (Issue #59): the four flows are certified to `frontend integrated` with per-flow evidence ("Feature 002 End-to-End Certification" section). Promotion to `flow complete` awaits the manual homologation round-trip (SC-006/T122).
