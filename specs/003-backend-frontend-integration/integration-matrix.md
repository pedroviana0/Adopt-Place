...
| REQUEST-OWNER-01 | Responsible dashboards still call mock list/detail/decision/completion functions from `solicitacoes.ts`; frontend replacement remains Issue #45. | Protected list/detail/decision/completion HTTP contracts revalidate the active responsible account, scope screening reads before selection, exclude CPF/address/user IDs, and enforce approval/refusal/completion transitions transactionally. | `SolicitacaoAdocao`, `Animal`, conversation entities, and the APROVADA/RECUSADA/CONCLUIDA transition rules. | Frontend mocks remain intentionally preserved until Issue #45. Process certification is blocked because T076 formally depends on incomplete T073. | Arthur consumes the documented protected DTOs only after the dependency exception is resolved. | Implementation validated, certification blocked. Evidence: Issues #43; 18 focused request tests within the 33-test health/request set; 401/inactive-account/ownership/privacy/repeated-transition and transactional side-effect coverage. |
| ANIMALS-CRUD-01 | Dashboard list/create/detail/update/delete uses protected HTTP. Create now persists the animal, uploads its selected primary photo and waits for a fresh protected read before reporting success; a failed upload keeps the existing animal ID for retry instead of creating a duplicate. Completed owner CRUD functions no longer use localStorage. | Protected animal handlers revalidate the active responsible account, derive exactly one owner server-side, prevent owner transfer and return allowlisted DTOs. | `Animal`, responsible profiles, taxonomy and exactly-one responsible rule. | The create/upload compensation path is implemented and covered locally, but real provider homologation is blocked until a valid backend `UPLOADTHING_TOKEN` is available. | Homologate create -> upload -> reload with an isolated database and provider credential before certifying T073. | `frontend integrated`, certification withheld. Evidence: Issues #35/#36/#39/#40/#41 plus local #42 implementation; focused unit tests and typechecks pass. |
| ANIMALS-PHOTOS-01 | `AnimalPhotosPanel.tsx` uploads new images through the canonical `/api/uploadthing` route and retains protected HTTP operations for reorder, primary selection and deletion. `AnimalForm.tsx` requires and confirms the first primary photo before create-flow success. | Upload middleware and completion revalidate active account, responsible role and current ownership; type/size limits are checked on both sides; a serializable transaction assigns first-primary/order; the only photo cannot be deleted. | `FotoAnimal`, max 10 photos, required primary photo and ordered gallery rule. | No valid `UPLOADTHING_TOKEN` is available for a real provider round trip, so implementation evidence cannot certify homologation or T073. | Configure the token only in an isolated backend environment, then prove owner create -> upload -> fresh read, unauthorized/inactive/wrong-role/nonowner rejection and existing-photo management. | `frontend integrated`, homologation pending. Evidence: 17 focused photo tests cover authorization, role/account state, ownership, file validation, provider failure, no fake metadata, persistence, first-primary, retry without duplication and fresh-read confirmation; all 184 backend tests, root lint/typecheck/Prisma validation/build, frontend typecheck, targeted semantic lint and frontend build pass. Full frontend lint remains blocked only by the pre-existing repository-wide CRLF/Prettier debt. |
| ANIMALS-RELATIONSHIPS-01 | `RelatedAnimalsPanel.tsx` lists candidates and links/unlinks through the real protected relationship contracts; completed relationship functions no longer use localStorage. | Protected list/link/unlink handlers require ownership of both animals; link and unlink update both directions transactionally, reject self-links and avoid duplicate pairs. | Paired `AnimalRelacionado` rows and ownership of both animals. | No flow-specific mock remains; only the separate photo-upload gap blocks T073. | Keep the owner-safe relationship DTO stable. | `flow complete`. Evidence: Issues #35/#38/#39/#41, PR #86, relationship transaction tests, frontend semantic lint and production build. |
| ANIMALS-SEARCH-01 | Dashboard filters and related-animal candidates call `GET /api/animais/gerenciados` with protected owner-scoped query parameters. | The contract combines `q`, status, taxonomy, porte and sexo while always retaining organization/foster ownership. | Indexed animal fields plus responsible ownership predicates. | No flow-specific mock remains; only the separate photo-upload gap blocks T073. | Keep the protected filter contract stable. | `flow complete`. Evidence: Issues #35/#39/#40/#41, PR #86, owner/filter tests, frontend semantic lint and production build. |
| HEALTH-BASIC-01 | `saude.ts`, `HealthPanel.tsx` and dashboard alerts remain mock until frontend Issue #46. | Protected health list/create/update/delete and 30-day alert contracts support the five Prisma history categories, reject `CONSULTA`, validate dates, revalidate ownership/account state and synchronize pending derived care transactionally. | `RegistroSaude`, `CuidadoPlanejado`, `TipoRegistroSaude`, owned `Animal`, and FR-017/FR-018/FR-051. | Frontend mocks remain intentionally preserved until Issue #46. Feature 002 expanded health remains outside this slice and still requires T083. Process certification is blocked because T076 formally depends on incomplete T073. | Arthur consumes only this feature 001 basic-health contract; expanded health waits for the feature 002 audit. | Implementation validated, certification blocked. Evidence: Issue #44; 15 focused health tests within the 33-test set; 401/wrong-role/inactive/ownership/date/CONSULTA/transaction/alert coverage. |
