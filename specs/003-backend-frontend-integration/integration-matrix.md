...
| FAVORITES-01 | `favoritos.ts` still accepts an `adotanteId` from browser callers and mutates localStorage until frontend Issue #33. | `GET /api/favoritos` and idempotent `PUT/DELETE /api/favoritos/[animalId]` revalidate the active adopter, derive identity from the session and return allowlisted animal summaries. | Composite `Favorito(adotanteId, animalId)` relation and ADOTANTE-only rule FR-014. | Frontend identity remains unsafe until #33 replaces the mock boundary; mocks must remain until that real flow passes. | Arthur may now consume the documented contracts without sending adopter/user IDs. | `backend ready`. Evidence: T053, T055-T057; Issue #32; focused authorization/ownership/idempotency tests; isolated PostgreSQL 16 HTTP round trip with one persisted favorite and idempotent add/remove. |
| REQUEST-ADOPTER-01 | `solicitacoes.ts` still creates adopter requests in localStorage; adopter routes list mock requests until frontend Issue #33. | `GET/POST /api/solicitacoes` and `GET /api/dashboard/adotante` derive the adopter from the active session, enforce screening/availability/duplicate guards and return narrow DTOs. | `SolicitacaoAdocao`, `Animal`, `Adotante`, `StatusSolicitacao`, screening/availability rules and composite uniqueness. | Client-side identity/transitions remain unsafe until #33; the schema permits only one request per adopter/animal, so non-active repeat behavior is not expanded without a later schema decision. | Arthur may now integrate adopter request/list/dashboard reads; owner decisions remain outside this flow. | `backend ready`. Evidence: T054-T057; Issue #32; 23 focused tests; typecheck, targeted lint and build; isolated PostgreSQL 16 HTTP round trip with 201 create, 409 active duplicate, own-list read and completed-screening dashboard. |
