...
| REQUEST-OWNER-01 | Responsible dashboards still call mock list/detail/decision/completion functions from `solicitacoes.ts`; frontend replacement remains Issue #45. | Protected list/detail/decision/completion HTTP contracts revalidate the active responsible account, scope screening reads before selection, exclude CPF/address/user IDs, and enforce approval/refusal/completion transitions transactionally. | `SolicitacaoAdocao`, `Animal`, conversation entities, and the APROVADA/RECUSADA/CONCLUIDA transition rules. | Frontend mocks remain intentionally preserved until Issue #45. | Arthur consumes the documented protected DTOs after the backend-ready boundary. | `backend ready`. Evidence: Issue #43; 18 focused request tests within the 33-test health/request set; 401/inactive-account/ownership/privacy/repeated-transition and transactional side-effect coverage. |
...
| HEALTH-BASIC-01 | `saude.ts`, `HealthPanel.tsx` and dashboard alerts remain mock until frontend Issue #46. | Protected health list/create/update/delete and 30-day alert contracts support the five Prisma history categories, reject `CONSULTA`, validate dates, revalidate ownership/account state and synchronize pending derived care transactionally. | `RegistroSaude`, `CuidadoPlanejado`, `TipoRegistroSaude`, owned `Animal`, and FR-017/FR-018/FR-051. | Frontend mocks remain intentionally preserved until Issue #46. Feature 002 expanded health remains outside this slice and still requires T083. | Arthur consumes only this feature 001 basic-health contract; expanded health waits for the feature 002 audit. | `backend ready`. Evidence: Issue #44; 15 focused health tests within the 33-test set; 401/wrong-role/inactive/ownership/date/CONSULTA/transaction/alert coverage. |
