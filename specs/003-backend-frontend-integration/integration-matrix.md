...
|------|----------------------------------|--------------------|----------------|
| Session and accounts | `lib/auth.ts`, `lib/actions/login.ts`, `lib/actions/auth-register.ts`, `lib/actions/admin-users.ts`, `lib/queries/admin-users.ts`, auth/admin schemas | `Usuario`, `Adotante`, `Organizacao`, `AcolhedorIndependente`, NextAuth models | Auth proof contract defined by Issue #20; implementation remains Issue #21. Registration/profile/admin contracts remain inventory only |
...
`contract defined`. AUTH-01 advances only because Issue #20 records exact
methods/paths, DTOs, errors, sensitive-field exclusions, tests, backend sources,
frontend dependency, and the same-origin/proxy decision. Implementation remains
Issue #21.
...
|---------|------------------------------------|---------------------------------|-----------------|--------------------|------------------------------------------------|---------------------|
| AUTH-01 | Login, session, reload, logout, and protected-route identity. `frontend/src/lib/data/sessao.ts` authenticates against plaintext seed passwords and persists `adoptplace:session:v1`; `_authenticated.tsx` depends on that client-only state. | `lib/auth.ts` configures NextAuth Credentials, JWT enrichment, active-account rejection, role and scoped profile IDs; `app/api/auth/[...nextauth]/route.ts` exports the existing GET/POST handlers; `lib/actions/auth-guards.ts` and `lib/permissions.ts` provide active-session and role checks. | `Usuario`, NextAuth `Account`/`Session` models, `TipoPerfil`, `Usuario.ativo`, and the clarified secure-cookie session rule. | **Critical**: the contracts are defined but not implemented or proven. The planned protected `GET /api/session` still needs tests/implementation; reload and SSR retain the mock null-session window until Issues #21/#22. | Implement Issue #21 tests and backend proof for active login, safe session after reload, logout, 401 without session, and inactive-user block. Inventory group: **Session/login/logout**. | `contract defined`. Evidence: `contracts/http-contract-inventory.md` AUTH-CSRF-01, AUTH-LOGIN-01, AUTH-SESSION-01, AUTH-LOGOUT-01; T015-T019; Issue #20. |
