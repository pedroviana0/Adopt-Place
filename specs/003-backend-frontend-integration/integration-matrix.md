...
| ANIMALS-CRUD-01 | Owner dashboard CRUD still uses `animais.ts` localStorage helpers until frontend Issue #40. | Protected `GET/POST /api/animais/gerenciados` and `GET/PATCH/DELETE /api/animais/gerenciados/[id]` revalidate the active responsible account, derive exactly one owner server-side, prevent owner transfer and return allowlisted DTOs. | `Animal`, responsible profiles, taxonomy and exactly-one responsible rule. | Frontend consumption and per-flow localStorage removal remain pending; create-before-upload remains the documented two-step flow. | Arthur integrates the CRUD contract in Issue #40 without changing its ownership boundary. | `backend ready`. Evidence: Issues #35/#36/#39; T061/T064-T066/T070; ownership, inactive-account, DTO privacy, schema and HTTP tests; `npm test`, `npm run lint`, `npm run typecheck`, `npm run build`, and `npx prisma validate` pass. No schema or migration change. |
| ANIMALS-PHOTOS-01 | `AnimalForm.tsx` still compresses photos to base64 and persists them in localStorage until frontend Issue #40. | Uploadthing completion and protected photo handlers revalidate current ownership; first upload becomes primary, later uploads receive the next order, reordering requires the complete contiguous set, and deleting a primary photo requires a replacement. | `FotoAnimal`, max 10 photos, required primary photo and ordered gallery rule. | Upload needs an existing animal ID; concurrent uploads still require serialization by the frontend because Prisma has no order uniqueness constraint. | Arthur replaces base64/localStorage only after the real upload and photo contract succeeds in Issue #40. | `backend ready`. Evidence: Issues #35/#37/#39; T062/T064-T065/T067/T070; primary/order/upload/ownership tests and the full validation commands recorded for ANIMALS-CRUD-01. No schema or migration change. |
| ANIMALS-RELATIONSHIPS-01 | `RelatedAnimalsPanel.tsx` still lists and mutates bidirectional links in localStorage until frontend Issue #41. | Protected list/link/unlink handlers require ownership of both animals; link and unlink update both directions transactionally, reject self-links and avoid duplicate pairs. | Paired `AnimalRelacionado` rows and ownership of both animals. | Frontend consumption and relationship mock removal remain pending. | Arthur consumes the owner-safe relationship DTO and mutations in Issue #41. | `backend ready`. Evidence: Issues #35/#38/#39; T063-T065/T068/T070; self-link, cross-owner, idempotency and bidirectional transaction tests plus the full validation commands recorded for ANIMALS-CRUD-01. |
| ANIMALS-SEARCH-01 | Related-animal candidates and dashboard list currently search localStorage. | `GET /api/animais/gerenciados` and the internal search action combine `q`, status, taxonomy, porte and sexo while always retaining organization/foster ownership; the former global unauthenticated search was removed. | Indexed animal fields plus responsible ownership predicates. | Frontend search/filter consumption remains pending in Issues #40/#41. | Arthur consumes the same protected filter contract for dashboard and relationship candidates. | `backend ready`. Evidence: Issues #35/#39; T064-T066/T069-T070; combined organization/foster filter tests, 401/403 guards, DTO privacy test and the full validation commands recorded for ANIMALS-CRUD-01. |
