...
- [ ] T001 [US5] [PEDRO] Criar matriz de baseline e comparação em `specs/004-ui-ux-redesign/quickstart.md` com rota, papel, estado, viewport 375/1024/1440, zoom, data e caminho de evidência, atendendo FR-003/SC-001/SC-002.
...
- [ ] T004 [US5] [PEDRO] Executar e registrar gate pré-implementação em `specs/004-ui-ux-redesign/quickstart.md`: `git diff --check`, builds/tipos disponíveis, débito CRLF/Prettier e ausência de mudança em `app/`, `lib/` e `prisma/` (FR-016/FR-017, CR-002–CR-007).

## Phase 2 — Fundação visual e tokens

- [ ] T005 [US5] [ARTHUR] Após T001–T004, inventariar usos de cor, tipografia, raio, sombra e foco em `frontend/src/styles.css` e `frontend/src/components/ui/` e documentar a decisão mínima em `specs/004-ui-ux-redesign/contracts/ui-patterns.md` (UX-06, FR-013–FR-015).
...
- [ ] T007 [US5] [ARTHUR] Aplicar foco visível contínuo e estados disabled/selected às primitives afetadas em `frontend/src/components/ui/button.tsx`, `frontend/src/components/ui/input.tsx`, `frontend/src/components/ui/select.tsx`, `frontend/src/components/ui/dialog.tsx`, `frontend/src/components/ui/dropdown-menu.tsx` e `frontend/src/components/ui/sheet.tsx` quando existentes (UX-10, FR-014/FR-015).
- [ ] T008 [US5] [PEDRO] Verificar contrastes dos tokens e estados definidos em `frontend/src/styles.css` e registrar resultados AA na matriz de `quickstart.md` (UX-06/UX-10, SC-007).
- [ ] T009 [US5] [ARTHUR] Executar o procedimento manual “Primitives e foco” de `specs/004-ui-ux-redesign/quickstart.md` após T006–T007, registrando papel, rota, controle, estado, viewport, resultado e evidência; não criar teste de componente incompatível com `vitest.config.ts` (FR-014/FR-015).
...
- [ ] T017 [US2] [ARTHUR] Executar o procedimento manual “Confirmação destrutiva” de `specs/004-ui-ux-redesign/quickstart.md` para organização e administrador, registrando rota, item, controle, viewport, cancelamento, confirmação, foco restaurado e evidência; não criar teste de componente incompatível com `vitest.config.ts` (UX-02, SC-004).
...
- [ ] T019 [PEDRO] [US2] Executar roteiro manual de sucesso, erro recuperável, cancelamento e confirmação em `/dashboard/documentos` e `/dashboard/admin/usuarios`, registrando 0 mutações no cancelamento (SC-004/SC-006).
...
- [ ] T021 [US3] [ARTHUR] Criar placeholder neutro para ausência de foto em `frontend/src/components/app/PublicAnimalCard.tsx`, usando somente dados reais atuais (UX-05, FR-010).
- [ ] T022 [US3] [ARTHUR] Melhorar feedback de carregamento, erro, vazio e limpeza de filtros em `frontend/src/components/app/AnimalFilters.tsx` sem mudar `frontend/src/lib/data/catalogos.ts` ou contratos (UX-07, FR-006/FR-009).
- [ ] T023 [US3] [ARTHUR] Aplicar skeleton/estados estáveis à home em `frontend/src/routes/index.tsx` (UX-04, FR-008/SC-005).
- [ ] T024 [ARTHUR] [P] [US3] Aplicar skeleton/estados estáveis à vitrine em `frontend/src/routes/vitrine.tsx` após T020–T022 (UX-03/UX-04/UX-07, FR-006–FR-009).
- [ ] T025 [ARTHUR] [P] [US3] Ajustar apresentação responsiva e foco do detalhe em `frontend/src/routes/animais.$animalId.tsx` após T006–T007, preservando DTO público (FR-014–FR-016).
- [ ] T026 [US3] [ARTHUR] Executar o procedimento manual “Descoberta pública” de `specs/004-ui-ux-redesign/quickstart.md`, registrando loading, filtro, vazio, placeholder, viewport e captura antes/depois; não criar teste de componente incompatível com `vitest.config.ts` (UX-03–UX-05, SC-005/SC-006).
- [ ] T027 [PEDRO] [US3] Executar roteiro manual público em `/` e `/vitrine`: 375/1024/1440, carregamento, filtro, vazio, card sem foto e detalhe; registrar comparação (SC-002/SC-005–SC-007).

## Phase 6 — Jornadas do adotante

**Goal**: US4 — jornadas autenticadas responsivas sem mudança de contrato.

- [ ] T028 [US4] [ARTHUR] Adaptar layout/estados de `/meu-perfil` em `frontend/src/routes/_authenticated.meu-perfil.tsx` para 375/1024/1440 consumindo apenas contratos atuais (FR-011/FR-016).
- [ ] T029 [ARTHUR] [P] [US4] Adaptar layout/feedback de `/triagem` em `frontend/src/routes/_authenticated.triagem.tsx` após T015–T018 (FR-006/FR-011).
- [ ] T030 [ARTHUR] [P] [US4] Adaptar vazio/erro/lista de `/meus-favoritos` em `frontend/src/routes/_authenticated.meus-favoritos.tsx` após T015–T018 (FR-007/FR-011).
...
- [ ] T033 [PEDRO] [US4] Executar regressão manual de adotante em perfil, triagem, favoritos, solicitação e chat ativo/arquivado, com teclado e viewport documentados em `quickstart.md` (SC-001–SC-008).

## Phase 7 — Jornadas de organização/acolhedor

**Goal**: US4 — operações de responsável legíveis, responsivas e seguras.

- [ ] T034 [US4] [ARTHUR] Adaptar lista e formulário de animais em `frontend/src/routes/_authenticated.dashboard.animais.index.tsx`, `frontend/src/routes/_authenticated.dashboard.animais.novo.tsx` e `frontend/src/components/app/AnimalForm.tsx`, preservando upload e taxonomia atuais (FR-006/FR-011/FR-016).
...
- [ ] T039 [PEDRO] [US4] Executar regressão manual separada de organização e acolhedor independente: criar/editar animal e foto, solicitação, saúde, documento, chat e estados de erro/cancelamento; registrar permissões/destinos e evidências próprias em `specs/004-ui-ux-redesign/quickstart.md` (SC-004/SC-006/SC-008).
...
- [ ] T043 [PEDRO] [US5] Auditar teclado e foco contínuo em todas as rotas da matriz de `quickstart.md`, registrando rota, papel, viewport, sequência e resultado (UX-01/UX-10, SC-001/SC-003).
- [ ] T044 [PEDRO] [US5] Auditar zoom 200%, rolagem horizontal e toque/pointer nas rotas principais em `quickstart.md` (NFR-001/NFR-002, SC-002).
- [ ] T045 [PEDRO] [US5] Auditar contraste e comunicação não exclusiva por cor de tokens, badges, foco, sucesso, aviso, erro e informação; registrar em `quickstart.md` (UX-06/UX-10, FR-013–FR-015).
- [ ] T046 [PEDRO] [US5] Executar leitor de tela quando disponível nas jornadas de menu, diálogo, filtros e card em `quickstart.md`; registrar limitação verificável se indisponível (NFR-003).
...
- [ ] T048 [PEDRO] [US5] Rodar validações automatizadas e regressão 003: `npm test`, typecheck, lint, Prisma validate, build raiz, build frontend e lint focado; separar débito CRLF/Prettier em `quickstart.md` (SC-008).
...
- [ ] T050 [US5] [PEDRO] Atualizar rastreabilidade UX → FR/SC → evidências em `specs/004-ui-ux-redesign/spec.md` e `tasks.md`, garantindo cobertura UX-01…UX-10.
...
- [ ] T052 [US5] [PEDRO] Executar revisão de escopo e `git diff --check`, confirmando que mudanças funcionais em `app/`, `lib/`, `prisma/`, contratos HTTP e `legacy/frontend-antigo/` estão ausentes (FR-016/FR-017, CR-002–CR-007).
...
| T010–T014 | T005–T009 | `frontend/src/components/app/Navbar.tsx`, `frontend/src/routes/__root.tsx`, `frontend/src/routes/_authenticated.dashboard.tsx` / Arthur | T015–T019 somente após C1 | C2 de destinos/foco; não paralelizar alterações do shell |
| T015–T019 | T005–T009 | `frontend/src/components/app/AsyncState.tsx`, `frontend/src/components/app/ConfirmDestructiveAction.tsx`, `frontend/src/components/app/EmptyState.tsx` / Arthur | T010–T014 e T025, após C1 | C2 de estado/confirmar; componentes publicados antes de consumidores |
| T020–T024, T026–T027 | T015, T018, T020–T022 conforme indicado | `frontend/src/components/app/AnimalShowcaseSkeleton.tsx`, `frontend/src/components/app/AnimalFilters.tsx`, `frontend/src/components/app/PublicAnimalCard.tsx` / Arthur | T025, T028 e T034–T038/T053 quando não alterarem esses arquivos | C3 público; T024 só após T020–T022 |
| T025 | T006–T007 e C2 de shell | `frontend/src/routes/animais.$animalId.tsx` / Arthur | T015–T024, T028–T042/T053 | rota isolada; não depende de todos os estados assíncronos |
| T028–T032 | C1 e, para estados, T015–T018 conforme a tarefa | rotas do adotante / Arthur | T034–T038/T053 em arquivos distintos | regressão por papel antes de C4; sem alterar shared components |
| T034–T038, T053 | C1 e, para estado/confirmação, T015–T018 | rotas/componentes de organização e acolhedor / Arthur | T028–T032 e entre si somente em arquivos distintos | regressão de organização e acolhedor; T037 e T053 são revisáveis separadamente |
| T040–T042 | T016, T018 e C1 | `frontend/src/routes/_authenticated.dashboard.admin.usuarios.tsx` / Arthur | rotas do adotante, organização e acolhedor, não T041 com T040 | lista e confirmação aprovadas; mesmo arquivo serializado |
...
1. Pedro: T001–T004; Arthur: T005–T009; Pedro: T008 em revisão de token.
...
- **C2** após T019: navegação e confirmação aprovadas por teclado.
- **C3** após T027: descoberta pública e screenshots aprovados.
- **C4** após T033/T039/T042: regressões por papel aprovadas.
- **C5** após T052: SC-001…SC-008, diff de escopo e comparação final aprovados.
...
| UX-02 | FR-004–FR-006 | SC-004 | 3, 6 e 7 | T016–T019, T041, T053 | procedimento de confirmação por organização/admin |
...
| UX-05 | FR-010 | SC-007 | 4 | T021, T026–T027 | card sem foto e captura comparável |
